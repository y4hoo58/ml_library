...
Kmeans::Kmeans(Data *_data, int _k_num,int _n_init ,int _max_iter) : Model(_data){
    ...
    vector<float> _clusters(data->train_size, 0.0);
    clusters = _clusters;
}
...
vector<vector<float>> Kmeans::init_centroids(){
    ...
    for(int i=0;i<k_num;i++){
        ...
        int _rand_index=Helpers::random_generator( data->train_size);

        // Assign it as a centroid
        _centroids.push_back(data->train_x->at(_rand_index));
    }
    return _centroids;
}
...
void Kmeans::find_cluster() {
    for(int i=0;i<data->train_size; i++){
        vector<float> _val = data->train_x->at(i);
        int _cluster_index=0;
        float _min_cluster_distance=0;
        for(int j=0;j<k_num;j++){
            float _distance = 0;
            for(int k=0;k<data->feature_size; k++){
                _distance = _distance + (float)pow((centroids.at(j).at(k) - data->train_x->at(i).at(k)), 2 );
            }
...
void Kmeans::calc_centroids(){
    ...
    for(int i=0;i<k_num;i++){
        vector<float> _centroid(data->feature_size, 0.0);
        float _counter =0;
        // Loop over all rows.
        // Find where i == cluster number.
        // Sum all rows column wise that belongs to ith cluster.
        for(int j=0;j<data->train_size; j++){
            if(clusters.at(j) == (float)i){
                _counter = _counter+1;
                for(int k=0;k<data->feature_size; k++){
                    _centroid.at(k) =_centroid.at(k)+data->train_x->at(j).at(k);
                }
            }
        }
        ...
        for(int j=0;j<data->feature_size; j++){
