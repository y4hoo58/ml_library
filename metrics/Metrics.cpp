...
float Metrics::mse(vector<float> *_targets,vector<float> *_predictions){
    float _error = 0;
...
float Metrics::mae(vector<float> *_targets,vector<float> *_predictions){
    float _error = 0;
    for(int i=0;i<_targets->size();i++){
...
float Metrics::binary_crossentropy(vector<float> *_targets, vector<float> *_predictions) {
    float _error =0;
    for(int i=0;i<_targets->size();i++){
...
float Metrics::entropy(vector<float> *_targets) {
...
float Metrics::rand_index(vector<float> *_vec1,vector<float> *_vec2){
    int _same_class=0;
    int _diff_class =0;
    int _total_pairs = 0;
